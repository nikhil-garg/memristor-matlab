...
function [result tot_it converged f_dx] = MEXP_Simple_Explicit_AC_Analysis_with_Compensate(SimParam, voltage, input, pre_input, step_size, first_time)  
    %perform AC analysis
    converged = 0;
    tot_it = 1;
    ...
    for i = 1:SimParam.max_iter
        ...
        %residue by xn+1        
        dx = (SimParam.B*input+f) - C*(next_voltage - voltage)/step_size - G*next_voltage - dx;
        
        %mexp residue
        dxdt = C\(-G*next_voltage + SimParam.B*input+f);
        f_dx = (SimParam.B*input+f) - Cnl*dxdt - Gnl*next_voltage;        
        
        if (norm(dx, 'inf') <= SimParam.iter_err_tol)
            converged = 1;
            tot_it = i;            
            break;
        end       
