	   ...
	   double temperature)
{
	...
	if (Temp != Tnom)
	{   
	    T0 = Eg0 / Vtm0 - Eg / model->BSIM3vtm + model->BSIM3jctTempExponent * log(Temp / Tnom);
	...
	if (Size_Not_Found)
	{   
	    pParam = (struct bsim3SizeDependParam *)malloc(sizeof(struct bsim3SizeDependParam));
	    if (pLastKnot == NULL)
		...
		//(*(SPfrontEnd->IFerror))(ERR_FATAL, "BSIM3: mosfet %s, model %s: Effective channel length <= 0", namarray);
		printf("BSIM3: mosfet and model: Effective channel length <= 0");
		...
		//namarray[0] = model->BSIM3modName;
		//namarray[1] = here->BSIM3name;
		//(*(SPfrontEnd->IFerror))(ERR_FATAL, "BSIM3: mosfet %s, model %s: Effective channel width <= 0", namarray);
		printf("BSIM3: mosfet and model: Effective channel width <= 0");
		return ERR_BAD_PARAMETER;
	    }
		...
		//namarray[0] = model->BSIM3modName;
		//namarray[1] = here->BSIM3name;
		//(*(SPfrontEnd->IFerror))(ERR_FATAL, "BSIM3: mosfet %s, model %s: Effective channel length for C-V <= 0", namarray);
		printf("BSIM3: mosfet and model: Effective channel length for C-V <= 0");
		return ERR_BAD_PARAMETER;
	    }
		...
		//namarray[0] = model->BSIM3modName;
		//namarray[1] = here->BSIM3name;
		//(*(SPfrontEnd->IFerror))(ERR_FATAL, "BSIM3: mosfet %s, model %s: Effective channel width for C-V <= 0", namarray);
		printf("BSIM3: mosfet and model: Effective channel width for C-V <= 0");
		return ERR_BAD_PARAMETER;
	    }
	    ...
	    pParam->BSIM3abulkCVfactor = 1.0 + pow((pParam->BSIM3clc / pParam->BSIM3leffCV), pParam->BSIM3cle);
	    ...
	    pParam->BSIM3u0temp = pParam->BSIM3u0 * pow(TRatio, pParam->BSIM3ute); 
	    ...
	    pParam->BSIM3phi = 2.0 * Vtm0 * log(pParam->BSIM3npeak / ni);

	    pParam->BSIM3sqrtPhi = sqrt(pParam->BSIM3phi);
	    pParam->BSIM3phis3 = pParam->BSIM3sqrtPhi * pParam->BSIM3phi;

	    pParam->BSIM3Xdep0 = sqrt(2.0 * EPSSI / (Charge_q * pParam->BSIM3npeak * 1.0e6)) * pParam->BSIM3sqrtPhi; 
	    pParam->BSIM3sqrtXdep0 = sqrt(pParam->BSIM3Xdep0);
	    pParam->BSIM3litl = sqrt(3.0 * pParam->BSIM3xj * model->BSIM3tox);
	    pParam->BSIM3vbi = Vtm0 * log(1.0e20 * pParam->BSIM3npeak / (ni * ni));
	    pParam->BSIM3cdep0 = sqrt(Charge_q * EPSSI * pParam->BSIM3npeak * 1.0e6 / 2.0 / pParam->BSIM3phi);

	    pParam->BSIM3ldeb = sqrt(EPSSI * Vtm0 / (Charge_q * pParam->BSIM3npeak * 1.0e6)) / 3.0;
		...
		if (!model->BSIM3gamma1Given)
		    pParam->BSIM3gamma1 = 5.753e-12 * sqrt(pParam->BSIM3npeak) / model->BSIM3cox;

		if (!model->BSIM3gamma2Given)
		    pParam->BSIM3gamma2 = 5.753e-12 * sqrt(pParam->BSIM3nsub) / model->BSIM3cox;

		T0 = pParam->BSIM3gamma1 - pParam->BSIM3gamma2;
		T1 = sqrt(pParam->BSIM3phi - pParam->BSIM3vbx) - pParam->BSIM3sqrtPhi;
		T2 = sqrt(pParam->BSIM3phi * (pParam->BSIM3phi - pParam->BSIM3vbm)) - pParam->BSIM3phi;
		pParam->BSIM3k2 = T0 * T1 / (2.0 * T2 + pParam->BSIM3vbm);
		pParam->BSIM3k1 = pParam->BSIM3gamma2 - 2.0 * pParam->BSIM3k2 * sqrt(pParam->BSIM3phi - pParam->BSIM3vbm);
	    }
		...
		{   
		    pParam->BSIM3vfb = model->BSIM3type * pParam->BSIM3vth0 - pParam->BSIM3phi 
	    ...
	    if (!model->BSIM3vth0Given)
	    {   
		pParam->BSIM3vth0 = model->BSIM3type * 
		    (pParam->BSIM3vfb + pParam->BSIM3phi + pParam->BSIM3k1 * pParam->BSIM3sqrtPhi);
	    }
	    ...
	    T1 = sqrt(EPSSI / EPSOX * model->BSIM3tox * pParam->BSIM3Xdep0);
	...
	here->BSIM3drainConductance = model->BSIM3sheetResistance * here->BSIM3drainSquares;
	if (here->BSIM3drainConductance > 0.0)
	    here->BSIM3drainConductance = 1.0 / here->BSIM3drainConductance;
	else
	    here->BSIM3drainConductance = 0.0;

	here->BSIM3sourceConductance = model->BSIM3sheetResistance * here->BSIM3sourceSquares;
	if (here->BSIM3sourceConductance > 0.0) 
	    here->BSIM3sourceConductance = 1.0 / here->BSIM3sourceConductance;
	else
	...
	{   
		...
		+ here->BSIM3sourcePerimeter * model->BSIM3jctSidewallTempSatCurDensity;
	}
	if ((SourceSatCurrent > 0.0) && (model->BSIM3ijth > 0.0))
	{   
	    here->BSIM3vjsm = Nvtm * log(model->BSIM3ijth / SourceSatCurrent + 1.0);
	    here->BSIM3IsEvjsm = SourceSatCurrent * exp(here->BSIM3vjsm / Nvtm);
	}
	...
	{   
		...
		+ here->BSIM3drainPerimeter * model->BSIM3jctSidewallTempSatCurDensity;
	}
	if ((DrainSatCurrent > 0.0) && (model->BSIM3ijth > 0.0))
	{   
	    here->BSIM3vjdm = Nvtm * log(model->BSIM3ijth / DrainSatCurrent + 1.0);
