...
/*
    Arguments: (IN) instances : bsim3 transistors list
	       (IN) voltage   : voltage value at each node
	       (OUT) b	      : right hand side vector for BE method 
	       (OUT) b_mexp   : right hand side vector for MEXP method
	       (IN) step_size : step size from current time instant
	       (IN) CKTmode   : internal state control variable (see BSIM3_Matlab_Usage.txt)
 */
int
BSIM3load(ModelInstance *instances, 
	  double *voltage,
	  double *b, 
	  double *b_mexp, 
	  double step_size, 
	  int CKTmode)
{
    ...
    /* parameters for matric C */
    double c_gcggb, c_gcgdb, c_gcgsb, c_gcdgb, c_gcddb, c_gcdsb;
    double c_gcsgb, c_gcsdb, c_gcssb, c_gcbgb, c_gcbdb, c_gcbsd;
    double c_gqdef, c_gcqgb, c_gcqdb, c_gcqsb, c_gcqbb, c_gcbsb;

    /* parameters for extrinsic matric C */
    double ec_gcggb, ec_gcgdb, ec_gcgsb, ec_gcdgb, ec_gcddb, ec_gcdsb;
    double ec_gcsgb, ec_gcsdb, ec_gcssb, ec_gcbgb, ec_gcbdb, ec_gcbsd;
    double ec_gqdef, ec_gcqgb, ec_gcqdb, ec_gcqsb, ec_gcqbb, ec_gcbsb;
    ...
    BSIM3model *model;
    BSIM3instance *here;
    ModelInstance *ptr = instances;
    
    ScalingFactor = 1.0e-9;
     
    //ckt struct parameters
    double CKTgmin;
    double CKTtemp;

    //CKTmode = (MODEINITFLOAT | MODETRANOP);
    //CKTmode = (MODEINITJCT | MODETRANOP);
    //CKTmode = (MODEINITFIX | MODETRANOP);
    CKTgmin = 1e-12;
    //step size 
    ag0 = CKTmode & MODETRANOP ? 0 : 1/step_size;

    ChargeComputationNeeded =  ((CKTmode & (MODEAC | MODETRAN | MODEINITSMSIG)) ||
			        ((CKTmode & MODETRANOP) && (CKTmode & MODEUIC))) ? 1 : 0;
    //initialize as zero
    //since stamp matrices are shared among all instance
    //only need to initialize one of them
    SetAllZero(ptr->instance->stamp);
    SetAllZero(ptr->instance->g_stamp);
    SetAllZero(ptr->instance->c_stamp);
    SetAllZero(ptr->instance->ec_stamp);

    for (ptr = instances; ptr; ptr = ptr->next)
    {    
	here = ptr->instance;
	model = here->BSIM3modPtr;
	CKTtemp = here->CKTtemp;

	///* check mode to see if propagate the internal state is required */
    	//if ((CKTmode & MODETRAN))
	//{
	//    memcpy(here->state2, here->state1, sizeof(double) * BSIM3numStates);
	//    memcpy(here->state1, here->state0, sizeof(double) * BSIM3numStates);
	//}
	...
	if ((CKTmode & MODEINITTRAN))
	{
	    memcpy(here->state2, here->state1, sizeof(double) * BSIM3numStates);
	    memcpy(here->state1, here->state0, sizeof(double) * BSIM3numStates);
	    memset(here->state0, 0, sizeof(double) * BSIM3numStates);
	} 
	else if ((CKTmode & MODEFIRSTITER))
	{
	    memcpy(here->state2, here->state1, sizeof(double) * BSIM3numStates);
	    memcpy(here->state1, here->state0, sizeof(double) * BSIM3numStates);
	}
	...
	//MODINITSMSIG --> initialize small signal mode
	//MODEINITJCT --> process junction voltage
	//MODEINITFIX --> process when device is OFF

	if ((CKTmode & MODEINITSMSIG))
	{   
	    vbs = *(here->state0 + BSIM3vbs);
	    vgs = *(here->state0 + BSIM3vgs);
	    vds = *(here->state0 + BSIM3vds);
	    qdef = *(here->state0 + BSIM3qdef);
	}
	else if ((CKTmode & MODEINITTRAN))
	{   
	    vbs = *(here->state1 + BSIM3vbs);
	    vgs = *(here->state1 + BSIM3vgs);
	    vds = *(here->state1 + BSIM3vds);
	    qdef = *(here->state1 + BSIM3qdef);
	}
	else if ((CKTmode & MODEINITJCT) && !here->BSIM3off) 
	{   
		...
		((CKTmode & (MODETRAN | MODEAC | MODEDCOP | MODEDCTRANCURVE)) || 
		(!(CKTmode & MODEUIC))))
	    {   
		vbs = 0.0;
	...
	{
	    ////disable the prediction feature temporary
	    //if ((CKTmode & MODEINITPRED))
	    //{   
	    //    xfact = ckt->CKTdelta / ckt->CKTdeltaOld[1];
	    //    *(here->state0 + here->BSIM3vbs) = *(here->state1 + here->BSIM3vbs);
	    //    vbs = (1.0 + xfact)* (*(here->state1 + here->BSIM3vbs)) 
	    //           - (xfact * (*(here->state2 + here->BSIM3vbs)));
	    //    *(here->state0 + here->BSIM3vgs) = *(here->state1 + here->BSIM3vgs);
	    //    vgs = (1.0 + xfact)* (*(here->state1 + here->BSIM3vgs))
	    //        - (xfact * (*(here->state2 + here->BSIM3vgs)));
	    //    *(here->state0 + here->BSIM3vds) = *(here->state1 + here->BSIM3vds);
	    //    vds = (1.0 + xfact)* (*(here->state1 + here->BSIM3vds))
	    //        - (xfact * (*(here->state2 + here->BSIM3vds)));
	    //    *(here->state0 + here->BSIM3vbd) = *(here->state0 + here->BSIM3vbs)
	    //    				   - *(here->state0 + here->BSIM3vds);
	    //    *(here->state0 + here->BSIM3qdef) = *(here->state1 + here->BSIM3qdef);
	    //    qdef = (1.0 + xfact)* (*(here->state1 + here->BSIM3qdef))
	    //    	-(xfact * (*(here->state2 + here->BSIM3qdef)));
	    //}
	    //else
	    //{
		vbs = model->BSIM3type * (*(voltage + here->BSIM3bNode)
	    				- *(voltage + here->BSIM3sNodePrime));
	    	vgs = model->BSIM3type * (*(voltage + here->BSIM3gNode) 
	    				- *(voltage + here->BSIM3sNodePrime));
	    	vds = model->BSIM3type * (*(voltage + here->BSIM3dNodePrime)
	    				- *(voltage + here->BSIM3sNodePrime));
	    	qdef = model->BSIM3type * (*(voltage + here->BSIM3qNode));
	    //}

	    vbd = vbs - vds;
	    vgd = vgs - vds;
	    vgdo = *(here->state0 + BSIM3vgs) - *(here->state0 + BSIM3vds);

	    //for by pass code (?)
	    delvbs = vbs - *(here->state0 + BSIM3vbs);
	    delvbd = vbd - *(here->state0 + BSIM3vbd);
	    delvgs = vgs - *(here->state0 + BSIM3vgs);
	    delvds = vds - *(here->state0 + BSIM3vds);
	    delvgd = vgd - vgdo;

	    if (here->BSIM3mode >= 0)
	    {   
		Idtot = here->BSIM3cd + here->BSIM3csub - here->BSIM3cbd;
		cdhat = Idtot - here->BSIM3gbd * delvbd + (here->BSIM3gmbs + here->BSIM3gbbs) * delvbs
			+ (here->BSIM3gm + here->BSIM3gbgs) * delvgs 
			+ (here->BSIM3gds + here->BSIM3gbds) * delvds;
		Ibtot = here->BSIM3cbs + here->BSIM3cbd - here->BSIM3csub;
		cbhat = Ibtot + here->BSIM3gbd * delvbd + (here->BSIM3gbs - here->BSIM3gbbs) * delvbs
		...
		Ibtot = here->BSIM3cbs + here->BSIM3cbd - here->BSIM3csub;
		cbhat = Ibtot + here->BSIM3gbs * delvbs + (here->BSIM3gbd - here->BSIM3gbbs) * delvbd
			- here->BSIM3gbgs * delvgd + here->BSIM3gbds * delvds;
	    }

	    // assume no by pass so far
	    ///* 
	    ...
	    //if ((!(CKTmode & MODEINITPRED)) && (ckt->CKTbypass))
	    //if ((fabs(delvbs) < (ckt->CKTreltol * MAX(fabs(vbs),
	    //    fabs(*(here->state0+here->BSIM3vbs))) + ckt->CKTvoltTol)))
	    //if ((fabs(delvbd) < (ckt->CKTreltol * MAX(fabs(vbd),
	    //    fabs(*(here->state0+here->BSIM3vbd))) + ckt->CKTvoltTol)))
	    //if ((fabs(delvgs) < (ckt->CKTreltol * MAX(fabs(vgs),
	    //    fabs(*(here->state0+here->BSIM3vgs))) + ckt->CKTvoltTol)))
	    //if ((fabs(delvds) < (ckt->CKTreltol * MAX(fabs(vds),
	    //    fabs(*(here->state0+here->BSIM3vds))) + ckt->CKTvoltTol)))
	    ...
	    //    {   
	    //        /* bypass code */
	    //        vbs = *(here->state0 + here->BSIM3vbs);
	    //        vbd = *(here->state0 + here->BSIM3vbd);
	    //        vgs = *(here->state0 + here->BSIM3vgs);
	    //        vds = *(here->state0 + here->BSIM3vds);
	    //        qdef = *(here->state0 + here->BSIM3qdef);
	    ...
	    von = here->BSIM3von;
	    if (*(here->state0 + BSIM3vds) >= 0.0)
	    {   
		vgs = DEVfetlim(vgs, *(here->state0 + BSIM3vgs), von);
		vds = vgs - vgd;
		vds = DEVlimvds(vds, *(here->state0 + BSIM3vds));
		vgd = vgs - vds;
	    }
	    else
	    {   
	    ...
	    if (vds >= 0.0)
	    {   
		vbs = DEVpnjlim(vbs, *(here->state0 + BSIM3vbs),
	    ...
	    {   
		vbd = DEVpnjlim(vbd, *(here->state0 + BSIM3vbd),
			CONSTvt0, model->BSIM3vcrit, &Check); 
		vbs = vbd + vds;
	    }
	}
	...
	{   
	    ...
	    {   
		evbs = exp(vbs / Nvtm);
		here->BSIM3gbs = SourceSatCurrent * evbs / Nvtm + CKTgmin;
		here->BSIM3cbs = SourceSatCurrent * (evbs - 1.0) + CKTgmin * vbs; 
	    }
	    else
	    {   
		...
		{   
		    ...
		    here->BSIM3gbs = T0 + CKTgmin;
		    here->BSIM3cbs = here->BSIM3IsEvjsm - SourceSatCurrent 
			+ T0 * (vbs - here->BSIM3vjsm) + CKTgmin * vbs;
		}
	    }
	}
	...
	{   
	    ...
	    {   
		evbd = exp(vbd / Nvtm);
		here->BSIM3gbd = DrainSatCurrent * evbd / Nvtm + CKTgmin;
		here->BSIM3cbd = DrainSatCurrent * (evbd - 1.0) + CKTgmin * vbd;
	    }
	    else
	    {   
	...
	TempRatio =  CKTtemp / model->BSIM3tnom - 1.0;
	...
	if ((pParam->BSIM3ngate > 1.e18) && (pParam->BSIM3ngate < 1.e25) && (Vgs > T0))
	...
	{   
	    T5 = Vgsteff / model->BSIM3tox * (pParam->BSIM3ua
		    + pParam->BSIM3uc * Vbseff + pParam->BSIM3ub * Vgsteff / model->BSIM3tox);
	    dDenomi_dVg = (pParam->BSIM3ua + pParam->BSIM3uc * Vbseff
		    + 2.0 * pParam->BSIM3ub * Vgsteff / model->BSIM3tox) / model->BSIM3tox;
	    dDenomi_dVd = 0.0;
	...
	else if (a1 > 0.0) /* Added to avoid the discontinuity 
			      problem caused by a1 and a2 (Lambda) */
	{   
	...
	{   
	    if (diffVds > pParam->BSIM3pscbe1 * pParam->BSIM3litl / EXP_THRESHOLD)
	    {   
	...
	else if (model->BSIM3capMod == 0)
	{
		...
		{   /* 50/50 partitioning */
			...
			here->BSIM3cggb = One_Third_CoxWL * (3.0 - dVdsat_dVg) * dVgs_eff_dVg;
			T2 = -One_Third_CoxWL * dVdsat_dVb;
			here->BSIM3cgsb = -(here->BSIM3cggb + T2);
			here->BSIM3cgdb = 0.0;
			...
			here->BSIM3cbgb = -(here->BSIM3cggb - Two_Third_CoxWL * dVgs_eff_dVg);
			T3 = -(T2 + Two_Third_CoxWL * dVth_dVb);
			here->BSIM3cbsb = -(here->BSIM3cbgb + T3);
			here->BSIM3cbdb = 0.0;
			here->BSIM3qinv = -(qgate + qbulk);
			...
			T5 = T3 / T1;
			here->BSIM3cggb = CoxWL * (1.0 - T5 * dVdsat_dVg) * dVgs_eff_dVg;
			tmp = -CoxWL * T5 * dVdsat_dVb;
			here->BSIM3cgdb = CoxWL * (T2 - 0.5 + 0.5 * T5);
			here->BSIM3cgsb = -(here->BSIM3cggb + here->BSIM3cgdb + tmp);
			...
			here->BSIM3cdgb = (T0 * dVdsat_dVg - T7 * dAlphaz_dVg) * dVgs_eff_dVg;
			T12 = T0 * dVdsat_dVb - T7 * dAlphaz_dVb;
			here->BSIM3cddb = T4 * (1.0 - 2.0 * T2 - T5);
			here->BSIM3cdsb = -(here->BSIM3cdgb + T12
				+ here->BSIM3cddb);

			here->BSIM3cbgb = -(here->BSIM3cggb + 2.0 * here->BSIM3cdgb);
			here->BSIM3cbdb = -(here->BSIM3cgdb + 2.0 * here->BSIM3cddb);
			here->BSIM3cbsb = -(here->BSIM3cgsb + 2.0 * here->BSIM3cdsb);
			here->BSIM3qinv = -(qgate + qbulk);
		    }
		}
	    }
	...
	{   
		...
		* pParam->BSIM3leffCV;
		...
		here->BSIM3cdsb = (Cgg + Cgd + Cgb + Cbg + Cbd + Cbb + Csg + Csd + Csb);
...
finished: 
	...
	{   
		...
		*(here->state0 + BSIM3qbs) = 0.0;
		...
		{   
		    ...
		    *(here->state0 + BSIM3qbs) = model->BSIM3PhiB * czbs 
			* (1.0 - arg * sarg) / (1.0 - MJ);
		...
		{   
		    ...
		    *(here->state0 + BSIM3qbs) += model->BSIM3PhiBSW * czbssw
		...
		{   
		    ...
		    *(here->state0 + BSIM3qbs) += model->BSIM3PhiBSWG * czbsswg
		...
		*(here->state0 + BSIM3qbs) = vbs * (T0 + 0.5 * T1);
		...
		*(here->state0 + BSIM3qbd) = 0.0;
		...
		{   
			...
			sarg = exp(-MJ * log(arg));
		    *(here->state0 + BSIM3qbd) = model->BSIM3PhiB * czbd 
			* (1.0 - arg * sarg) / (1.0 - MJ);
		...
		{   
			...
			sarg = exp(-MJSW * log(arg));
		    *(here->state0 + BSIM3qbd) += model->BSIM3PhiBSW * czbdsw 
			* (1.0 - arg * sarg) / (1.0 - MJSW);
		...
		{   
			...
			sarg = exp(-MJSWG * log(arg));
		    *(here->state0 + BSIM3qbd) += model->BSIM3PhiBSWG * czbdswg
			* (1.0 - arg * sarg) / (1.0 - MJSWG);
		...
		*(here->state0 + BSIM3qbd) = vbd * (T0 + 0.5 * T1);
	 ...
	 *  check convergence
	 */
	//if ((here->BSIM3off == 0) || (!(CKTmode & MODEINITFIX)))
	//{   
	//    if (Check == 1)
	//    {   
	//	ckt->CKTnoncon++;
	...
	*(here->state0 + BSIM3vbs) = vbs;
	*(here->state0 + BSIM3vbd) = vbd;
	*(here->state0 + BSIM3vgs) = vgs;
	*(here->state0 + BSIM3vds) = vds;
	*(here->state0 + BSIM3qdef) = qdef;
...
line755:
	...
	{   
		...
		/* without step size */
		c_gcggb = (here->BSIM3cggb + cgdo + cgso + pParam->BSIM3cgbo );
		c_gcgdb = (here->BSIM3cgdb - cgdo);
		c_gcgsb = (here->BSIM3cgsb - cgso);

		c_gcdgb = (here->BSIM3cdgb - cgdo);
		c_gcddb = (here->BSIM3cddb + here->BSIM3capbd + cgdo);
		c_gcdsb = here->BSIM3cdsb;

		c_gcsgb = -(here->BSIM3cggb + here->BSIM3cbgb + here->BSIM3cdgb + cgso);
		c_gcsdb = -(here->BSIM3cgdb + here->BSIM3cbdb + here->BSIM3cddb);
		c_gcssb = (here->BSIM3capbs + cgso - (here->BSIM3cgsb + here->BSIM3cbsb + here->BSIM3cdsb));

		c_gcbgb = (here->BSIM3cbgb - pParam->BSIM3cgbo);
		c_gcbdb = (here->BSIM3cbdb - here->BSIM3capbd);
		c_gcbsb = (here->BSIM3cbsb - here->BSIM3capbs);

		/* without intrinic cap */
		ec_gcggb = (cgdo + cgso + pParam->BSIM3cgbo);
		ec_gcgdb = (-cgdo);
		ec_gcgsb = (-cgso);
		...
		ec_gcbgb = (-pParam->BSIM3cgbo);
		ec_gcbdb = (-here->BSIM3capbd);
		ec_gcbsb = (-here->BSIM3capbs);
		...
		/* without step size */
		c_gqdef = ScalingFactor;

		c_gcqgb = here->BSIM3cqgb;
		c_gcqdb = here->BSIM3cqdb;
		c_gcqsb = here->BSIM3cqsb;
		c_gcqbb = here->BSIM3cqbb;

		c_gcggb = (cgdo + cgso + pParam->BSIM3cgbo );
		c_gcgdb = -cgdo;
		c_gcgsb = -cgso;

		c_gcdgb = -cgdo;
		c_gcddb = (here->BSIM3capbd + cgdo);

		c_gcsgb = -cgso;
		c_gcssb = (here->BSIM3capbs + cgso);

		c_gcbgb = -pParam->BSIM3cgbo;
		c_gcbdb = -here->BSIM3capbd;
		c_gcbsb = -here->BSIM3capbs;
	...
	{   
		...
		/* without step size */
		c_gcggb = (here->BSIM3cggb + cgdo + cgso + pParam->BSIM3cgbo);
		c_gcgdb = (here->BSIM3cgsb - cgdo);
		c_gcgsb = (here->BSIM3cgdb - cgso);

		c_gcdgb = -(here->BSIM3cggb + here->BSIM3cbgb + here->BSIM3cdgb + cgdo);
		c_gcddb = (here->BSIM3capbd + cgdo - (here->BSIM3cgsb + here->BSIM3cbsb + here->BSIM3cdsb));
		c_gcdsb = -(here->BSIM3cgdb + here->BSIM3cbdb + here->BSIM3cddb);

		c_gcsgb = (here->BSIM3cdgb - cgso);
		c_gcsdb = here->BSIM3cdsb;
		c_gcssb = (here->BSIM3cddb + here->BSIM3capbs + cgso);

		c_gcbgb = (here->BSIM3cbgb - pParam->BSIM3cgbo);
		c_gcbdb = (here->BSIM3cbsb - here->BSIM3capbd);
		c_gcbsb = (here->BSIM3cbdb - here->BSIM3capbs);

		/* without intrinsic cap */
		ec_gcggb = (cgdo + cgso + pParam->BSIM3cgbo);
		ec_gcgdb = (-cgdo);
		ec_gcgsb = (-cgso);
		...
		c_gcqgb = here->BSIM3cqgb;
		c_gcqdb = here->BSIM3cqsb;
		c_gcqsb = here->BSIM3cqdb;
		c_gcqbb = here->BSIM3cqbb;
		...
		{   
		    ...
		    Csg = here->BSIM3cdgb;
		    Cdg = -(here->BSIM3cggb + here->BSIM3cdgb + here->BSIM3cbgb);
		    dsxpart_dVg = (Csg - sxpart * (Csg + Cdg)) / qcheq;

		    Csd = here->BSIM3cdsb;
		    Cdd = -(here->BSIM3cgsb + here->BSIM3cdsb + here->BSIM3cbsb);
	...
	*(here->state0 + BSIM3qg) = qgate;
	*(here->state0 + BSIM3qd) = qdrn - *(here->state0 + BSIM3qbd);
	*(here->state0 + BSIM3qb) = qbulk + *(here->state0 + BSIM3qbd) + *(here->state0 + BSIM3qbs);

	if (here->BSIM3nqsMod)
	{   
	    *(here->state0 + BSIM3qcdump) = qdef * ScalingFactor;
	    *(here->state0 + BSIM3qcheq) = qcheq;
	}
	...
	if (CKTmode & MODEINITTRAN)
	{   
	    *(here->state1 + BSIM3qb) = *(here->state0 + BSIM3qb);
	    *(here->state1 + BSIM3qg) = *(here->state0 + BSIM3qg);
	    *(here->state1 + BSIM3qd) = *(here->state0 + BSIM3qd);
	    if (here->BSIM3nqsMod)
	    {   
		*(here->state1 + BSIM3qcheq) = *(here->state0 + BSIM3qcheq);
		*(here->state1 + BSIM3qcdump) = *(here->state0 + BSIM3qcdump);
	    }
	}

	error = NIintegrate(here, ag0, &geq, &ceq, 0.0, BSIM3qb);
	if (error)
	    return(error);

	error = NIintegrate(here, ag0, &geq, &ceq, 0.0, BSIM3qg);
	if (error)
	    return(error);

	error = NIintegrate(here, ag0, &geq, &ceq, 0.0, BSIM3qd);
	if (error)
	    return(error);

	if (here->BSIM3nqsMod)
	{   
	    error = NIintegrate(here, ag0, &geq, &ceq, 0.0, BSIM3qcdump);
	    if (error)
		return(error);

	    error = NIintegrate(here, ag0, &geq, &ceq, 0.0, BSIM3qcheq);
	    if (error)
		return(error);
	}
...
line860:
	...
	cqgate = *(here->state0 + BSIM3cqg);
	cqbulk = *(here->state0 + BSIM3cqb);
	cqdrn = *(here->state0 + BSIM3cqd);
	...
	{   
	    ...
	    ceqqd -= dxpart * T0 + T1 * (ddxpart_dVg * vgb - ddxpart_dVd
		    * vbd - ddxpart_dVs * vbs);
	    cqdef = *(here->state0 + BSIM3cqcdump) - gqdef * qdef;
	    cqcheq = *(here->state0 + BSIM3cqcheq)
		- (gcqgb * vgb - gcqdb * vbd  - gcqsb * vbs) + T0;
	}

	if (CKTmode & MODEINITTRAN)
	{   
	    *(here->state1 + BSIM3cqb) =
		*(here->state0 + BSIM3cqb);
	    *(here->state1 + BSIM3cqg) =
		*(here->state0 + BSIM3cqg);
	    *(here->state1 + BSIM3cqd) =
		*(here->state0 + BSIM3cqd);

	    if (here->BSIM3nqsMod)
	    {   
		*(here->state1 + BSIM3cqcheq) = 
		    *(here->state0 + BSIM3cqcheq);
		*(here->state1 + BSIM3cqcdump) = 
		    *(here->state0 + BSIM3cqcdump);
	    }
	}
...
line900:
	...
	{   
	    ...
	    //for MEXP implicit way, calculate i_q(x)
	    cqgate = -cqgate; //associated with ceqqg
	    cqbulk = -cqbulk; //associated with ceqqb
	    cqdrn = -cqdrn; //associated with ceqqd
	}

	//change ckt->CKTrhs into b
	m = here->BSIM3m;
	(*(b + here->BSIM3gNode) -= m * ceqqg);
	(*(b + here->BSIM3bNode) -= m * (ceqbs + ceqbd + ceqqb));
	(*(b + here->BSIM3dNodePrime) += m * (ceqbd - cdreq - ceqqd));
	(*(b + here->BSIM3sNodePrime) += m * (cdreq + ceqbs + ceqqg + ceqqb + ceqqd));
	
	//for MEXP method 
	(*(b_mexp + here->BSIM3bNode) -= m * (ceqbs + ceqbd));
	(*(b_mexp + here->BSIM3dNodePrime) += m * (ceqbd - cdreq));
	(*(b_mexp + here->BSIM3sNodePrime) += m * (cdreq + ceqbs));

	////for MEXP method with charge conservation q terms included 
	//(*(b + here->BSIM3gNode) -= m * cqgate);
	//(*(b_mexp + here->BSIM3bNode) -= m * (ceqbs + ceqbd + cqbulk));
	//(*(b_mexp + here->BSIM3dNodePrime) += m * (ceqbd - cdreq - cqdrn));
	//(*(b_mexp + here->BSIM3sNodePrime) += m * (cdreq + ceqbs + cqgate + cqbulk - cqdrn));


	//for MEXP implicit method, b = ids(x) + i_q(x)
	//(*(b + here->BSIM3gNode) -= m * -cqgate);
	//(*(b_mexp + here->BSIM3bNode) -= m * (ceqbs + ceqbd - cqbulk));
	//(*(b_mexp + here->BSIM3dNodePrime) += m * (ceqbd-cdreq + cqdrn));
	//(*(b_mexp + here->BSIM3sNodePrime) += m * (cdreq + ceqbs - cqgate - cqbulk - cqdrn));
	...
	//test
	//printf("-g:%d %e\n", here->BSIM3gNode, ceqqg);
	//printf("-b:%d %e\n", here->BSIM3bNode, ceqbs + ceqbd + ceqqb);
	//printf("-d:%d %e\n", here->BSIM3dNodePrime, -cdreq);
	//printf("-s:%d %e\n", here->BSIM3sNodePrime, cdreq);

	if (here->BSIM3nqsMod)
	    *(b + here->BSIM3qNode) += m * (cqcheq - cqdef);
	/*
	 *  load y matrix
	 */
	...
	#define GetPtrValue(ptr) *(ptr->element->value)

	(GetPtrValue(here->BSIM3DdPtr) += m * here->BSIM3drainConductance);
	(GetPtrValue(here->BSIM3GgPtr) += m * (gcggb - ggtg));
	(GetPtrValue(here->BSIM3SsPtr) += m * here->BSIM3sourceConductance);
	(GetPtrValue(here->BSIM3BbPtr) += m * (here->BSIM3gbd + here->BSIM3gbs
				     - gcbgb - gcbdb - gcbsb - here->BSIM3gbbs));
	(GetPtrValue(here->BSIM3DPdpPtr) += m * (here->BSIM3drainConductance
				       + here->BSIM3gds + here->BSIM3gbd
				       + RevSum + gcddb + dxpart * ggtd 
				       + T1 * ddxpart_dVd + gbdpdp));
	(GetPtrValue(here->BSIM3SPspPtr) += m * (here->BSIM3sourceConductance
				       + here->BSIM3gds + here->BSIM3gbs
				       + FwdSum + gcssb + sxpart * ggts
				       + T1 * dsxpart_dVs + gbspsp));
	(GetPtrValue(here->BSIM3DdpPtr) -= m * here->BSIM3drainConductance);
	(GetPtrValue(here->BSIM3GbPtr) -= m * (gcggb + gcgdb + gcgsb + ggtb));
	(GetPtrValue(here->BSIM3GdpPtr) += m * (gcgdb - ggtd));
	(GetPtrValue(here->BSIM3GspPtr) += m * (gcgsb - ggts));
	(GetPtrValue(here->BSIM3SspPtr) -= m * here->BSIM3sourceConductance);
	(GetPtrValue(here->BSIM3BgPtr) += m * (gcbgb - here->BSIM3gbgs));
	(GetPtrValue(here->BSIM3BdpPtr) += m * (gcbdb - here->BSIM3gbd + gbbdp));
	(GetPtrValue(here->BSIM3BspPtr) += m * (gcbsb - here->BSIM3gbs + gbbsp));
	(GetPtrValue(here->BSIM3DPdPtr) -= m * here->BSIM3drainConductance);
	(GetPtrValue(here->BSIM3DPgPtr) += m * (Gm + gcdgb + dxpart * ggtg 
	       		      + T1 * ddxpart_dVg + gbdpg));
	(GetPtrValue(here->BSIM3DPbPtr) -= m * (here->BSIM3gbd - Gmbs + gcdgb + gcddb
	       		      + gcdsb - dxpart * ggtb
	       		      - T1 * ddxpart_dVb - gbdpb));
	(GetPtrValue(here->BSIM3DPspPtr) -= m * (here->BSIM3gds + FwdSum - gcdsb
							    - dxpart * ggts - T1 * ddxpart_dVs - gbdpsp));
	(GetPtrValue(here->BSIM3SPgPtr) += m * (gcsgb - Gm + sxpart * ggtg 
	       		      + T1 * dsxpart_dVg + gbspg));
	(GetPtrValue(here->BSIM3SPsPtr) -= m * here->BSIM3sourceConductance);
	(GetPtrValue(here->BSIM3SPbPtr) -= m * (here->BSIM3gbs + Gmbs + gcsgb + gcsdb
	       		      + gcssb - sxpart * ggtb
	       		      - T1 * dsxpart_dVb - gbspb));
	(GetPtrValue(here->BSIM3SPdpPtr) -= m * (here->BSIM3gds + RevSum - gcsdb
				       - sxpart * ggtd - T1 * dsxpart_dVd - gbspdp));

	
	/* added by Shih-Hung */
	(GetPtrValue(here->G_BSIM3DdPtr) += m * here->BSIM3drainConductance);
	(GetPtrValue(here->G_BSIM3GgPtr) += m * -ggtg);
	(GetPtrValue(here->G_BSIM3SsPtr) += m * here->BSIM3sourceConductance);
	(GetPtrValue(here->G_BSIM3BbPtr) += m * (here->BSIM3gbd + here->BSIM3gbs - here->BSIM3gbbs));
	(GetPtrValue(here->G_BSIM3DPdpPtr) += m * (here->BSIM3drainConductance + here->BSIM3gds + here->BSIM3gbd + RevSum 
						+ dxpart * ggtd + T1 * ddxpart_dVd + gbdpdp));
	(GetPtrValue(here->G_BSIM3SPspPtr) += m * (here->BSIM3sourceConductance + here->BSIM3gds + here->BSIM3gbs + FwdSum 
						+ sxpart * ggts + T1 * dsxpart_dVs + gbspsp));
	(GetPtrValue(here->G_BSIM3DdpPtr) -= m * here->BSIM3drainConductance);
	(GetPtrValue(here->G_BSIM3GbPtr) -= m * ggtb);
	(GetPtrValue(here->G_BSIM3GdpPtr) += m * -ggtd);
	(GetPtrValue(here->G_BSIM3GspPtr) += m * -ggts);
	(GetPtrValue(here->G_BSIM3SspPtr) -= m * here->BSIM3sourceConductance);
	(GetPtrValue(here->G_BSIM3BgPtr) += m * -here->BSIM3gbgs);
	(GetPtrValue(here->G_BSIM3BdpPtr) += m * (-here->BSIM3gbd + gbbdp));
	(GetPtrValue(here->G_BSIM3BspPtr) += m * (-here->BSIM3gbs + gbbsp));
	(GetPtrValue(here->G_BSIM3DPdPtr) -= m * here->BSIM3drainConductance);
	(GetPtrValue(here->G_BSIM3DPgPtr) += m * (Gm + dxpart * ggtg + T1 * ddxpart_dVg + gbdpg));
	(GetPtrValue(here->G_BSIM3DPbPtr) -= m * (here->BSIM3gbd - Gmbs - dxpart * ggtb - T1 * ddxpart_dVb - gbdpb));
	(GetPtrValue(here->G_BSIM3DPspPtr) -= m * (here->BSIM3gds + FwdSum - dxpart * ggts - T1 * ddxpart_dVs - gbdpsp));
	(GetPtrValue(here->G_BSIM3SPgPtr) += m * (-Gm + sxpart * ggtg + T1 * dsxpart_dVg + gbspg));
	(GetPtrValue(here->G_BSIM3SPsPtr) -= m * here->BSIM3sourceConductance);
	(GetPtrValue(here->G_BSIM3SPbPtr) -= m * (here->BSIM3gbs + Gmbs + - sxpart * ggtb - T1 * dsxpart_dVb - gbspb));
	(GetPtrValue(here->G_BSIM3SPdpPtr) -= m * (here->BSIM3gds + RevSum - sxpart * ggtd - T1 * dsxpart_dVd - gbspdp));

	(GetPtrValue(here->C_BSIM3GgPtr) += m * c_gcggb);
	(GetPtrValue(here->C_BSIM3BbPtr) += m * (-c_gcbgb - c_gcbdb - c_gcbsb));
	(GetPtrValue(here->C_BSIM3DPdpPtr) += m * c_gcddb);
	(GetPtrValue(here->C_BSIM3SPspPtr) += m * c_gcssb);
	(GetPtrValue(here->C_BSIM3GbPtr) -= m * (c_gcggb + c_gcgdb + c_gcgsb));
	(GetPtrValue(here->C_BSIM3GdpPtr) += m * c_gcgdb);
	(GetPtrValue(here->C_BSIM3GspPtr) += m * c_gcgsb);
	(GetPtrValue(here->C_BSIM3BgPtr) += m * c_gcbgb);
	(GetPtrValue(here->C_BSIM3BdpPtr) += m * c_gcbdb);
	(GetPtrValue(here->C_BSIM3BspPtr) += m * c_gcbsb);
	(GetPtrValue(here->C_BSIM3DPgPtr) += m * c_gcdgb);
	(GetPtrValue(here->C_BSIM3DPbPtr) -= m * (c_gcdgb + c_gcddb + c_gcdsb));
	(GetPtrValue(here->C_BSIM3DPspPtr) -= m * (-c_gcdsb));
	(GetPtrValue(here->C_BSIM3SPgPtr) += m * c_gcsgb);
	(GetPtrValue(here->C_BSIM3SPbPtr) -= m * (c_gcsgb + c_gcsdb + c_gcssb));
	(GetPtrValue(here->C_BSIM3SPdpPtr) -= m * (-c_gcsdb));

	(GetPtrValue(here->EC_BSIM3GgPtr) += m * ec_gcggb);
	(GetPtrValue(here->EC_BSIM3BbPtr) += m * (-ec_gcbgb - ec_gcbdb - ec_gcbsb));
	(GetPtrValue(here->EC_BSIM3DPdpPtr) += m * ec_gcddb);
	(GetPtrValue(here->EC_BSIM3SPspPtr) += m * ec_gcssb);
	(GetPtrValue(here->EC_BSIM3GbPtr) -= m * (ec_gcggb + ec_gcgdb + ec_gcgsb));
	(GetPtrValue(here->EC_BSIM3GdpPtr) += m * ec_gcgdb);
	(GetPtrValue(here->EC_BSIM3GspPtr) += m * ec_gcgsb);
	(GetPtrValue(here->EC_BSIM3BgPtr) += m * ec_gcbgb);
	(GetPtrValue(here->EC_BSIM3BdpPtr) += m * ec_gcbdb);
	(GetPtrValue(here->EC_BSIM3BspPtr) += m * ec_gcbsb);
	(GetPtrValue(here->EC_BSIM3DPgPtr) += m * ec_gcdgb);
	(GetPtrValue(here->EC_BSIM3DPbPtr) -= m * (ec_gcdgb + ec_gcddb));
	(GetPtrValue(here->EC_BSIM3SPgPtr) += m * ec_gcsgb);
	(GetPtrValue(here->EC_BSIM3SPbPtr) -= m * (ec_gcsgb + ec_gcssb));



	if (here->BSIM3nqsMod)
	{   
	    GetPtrValue(here->BSIM3QqPtr) += m * (gqdef + here->BSIM3gtau);

	    GetPtrValue(here->BSIM3DPqPtr) += m * (dxpart * here->BSIM3gtau);
	    GetPtrValue(here->BSIM3SPqPtr) += m * (sxpart * here->BSIM3gtau);
	    GetPtrValue(here->BSIM3GqPtr) -= m * here->BSIM3gtau;

	    GetPtrValue(here->BSIM3QgPtr) += m * (ggtg - gcqgb);
	    GetPtrValue(here->BSIM3QdpPtr) += m * (ggtd - gcqdb);
	    GetPtrValue(here->BSIM3QspPtr) += m * (ggts - gcqsb);
	    GetPtrValue(here->BSIM3QbPtr) += m * (ggtb - gcqbb);

	    /* added by Shih-Hung */
	    GetPtrValue(here->G_BSIM3QqPtr) += m * here->BSIM3gtau;
	    GetPtrValue(here->G_BSIM3DPqPtr) += m * (dxpart * here->BSIM3gtau);
	    GetPtrValue(here->G_BSIM3SPqPtr) += m * (sxpart * here->BSIM3gtau);
	    GetPtrValue(here->G_BSIM3GqPtr) -= m * here->BSIM3gtau;
	    GetPtrValue(here->G_BSIM3QgPtr) += m * ggtg;
	    GetPtrValue(here->G_BSIM3QdpPtr) += m * ggtd;
	    GetPtrValue(here->G_BSIM3QspPtr) += m * ggts;
	    GetPtrValue(here->G_BSIM3QbPtr) += m * ggtb;

	    GetPtrValue(here->C_BSIM3QqPtr) += m * c_gqdef;
	    GetPtrValue(here->C_BSIM3QgPtr) += m * -c_gcqgb;
	    GetPtrValue(here->C_BSIM3QdpPtr) += m * -c_gcqdb;
	    GetPtrValue(here->C_BSIM3QspPtr) += m * -c_gcqsb;
	    GetPtrValue(here->C_BSIM3QbPtr) += m * -c_gcqbb;
	}
